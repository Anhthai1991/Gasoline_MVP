...
CSV_FILE = os.path.join(SCRIPT_DIR, "pvoil_gasoline_prices_full.csv")
...
def git_commit_push():
    ...
    Uses github-actions[bot] identity when running in GitHub Actions.
    """
    try:
        ...
        if is_ci:
            ...
            git_user = 'PVOIL Auto-Update Bot'
            git_email = 'bot@pvoil-update.local'

        # Configure git
        subprocess.run(['git', 'config', 'user.name', git_user],
                      check=False, capture_output=True, cwd=SCRIPT_DIR)
...
def main():
    ...
    if not all_data:
        print("\n[INFO] No new price data to update. Data is already up-to-date.")
        print("="*60 + "\n")
        sys.exit(0)
